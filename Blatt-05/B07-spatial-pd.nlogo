...
to interact  ;; patch procedure
  ...
  let neigh one-of neighbors

  ifelse (getStrategy = "c") [
    ifelse ([getStrategy] of neigh) = "c"
    [
      set aggPoints fput payoff-r aggPoints
      ask neigh [set aggPoints fput payoff-r aggPoints]
    ]
    [
      set aggPoints fput payoff-s aggPoints
      ask neigh [set aggPoints fput payoff-t aggPoints]
      set betrayed true
    ]
  ]
  [
    ifelse ([getStrategy] of neigh) = "c"
    [
      set aggPoints fput payoff-t aggPoints
      ask neigh [
        set aggPoints fput payoff-s aggPoints
        set betrayed true
      ]
    ...
    [
      set aggPoints fput payoff-p aggPoints
      set betrayed true
      ask neigh [
        set aggPoints fput payoff-p aggPoints
        set betrayed true
  ...
  set aggPoints remove-item 8 aggPoints
  ask neigh [set aggPoints remove-item 8 aggPoints]
end

to-report getStrategy
  if strategy = "random" [
   report one-of ["c" "d"]
  ]
...
to select-strategy  ;; patch procedure
  let bestNeigh one-of neighbors
  ask neighbors [
    if (sum aggPoints > [sum aggPoints] of bestNeigh) [
      set bestNeigh myself
    ]
  ]
  set strategy [strategy] of bestNeigh
end
...
"Yellow" 1.0 0 -987046 true "" "plot count patches with [ pcolor = yellow ]"
"Orange" 1.0 0 -955883 true "" "plot count patches with [ pcolor = orange ]"
